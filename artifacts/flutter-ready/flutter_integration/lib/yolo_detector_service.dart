...
class YoloDetectorService {
  ...
  Future<void> init({
    ...
    final labels = await File(labelsPath)
        .readAsLines()
        .then((lines) => lines.map((l) => l.trim()).where((l) => l.isNotEmpty).toList());
  ...
  List<List<double>> _flattenToRows(dynamic raw) {
    ...
    if (raw is List && raw.isNotEmpty) {
      if (raw.first is List && (raw.first as List).isNotEmpty && (raw.first as List).first is List) {
        final batch0 = (raw.first as List);
        return batch0.map((e) => (e as List).map((v) => (v as num).toDouble()).toList()).toList();
      }
      if (raw.first is List) {
        return raw.map((e) => (e as List).map((v) => (v as num).toDouble()).toList()).toList();
      }
    }
